...
const SIMPLE_HTML_FORM = fs.readFileSync("form.html").toString();
const APP_ID = "1203211791807545455";
const key = crypto
    .createHash('sha512')
...
export async function invokeLinking(req, res) {
    if (req.method === "GET") {
        if (req.query['code']) {
            ...
            if (response.ok) {
                ...
                res.cookie("dstk", encryptKey(data.access_token), {
                    expires: new Date(Date.now() + 12000 * 60 * 60),
                    sameSite: 'strict',
                    secure: true
                });
            }
...
function getHi3Region(uid) {
    ...
    else if (uid > 100000000 && uid < 200000000) {
        return 'NA';
    }
    else if (uid > 200000000 && uid < 300000000) {
        return 'EU';
    }
    else {
        return 'UNKNOWN';
